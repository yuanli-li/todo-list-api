...
def simulate(df, initial_lower, initial_upper, step, capital, fee_rate, ma_period, verbose=False):
    """
    动态网格（最终完美版）：
    - 实现了产品描述中的“事件驱动”均线涨幅计算逻辑。
    - reference_ma 的初始化等待第一个有效的MA值出现，确保逻辑严谨。
    - 边界突破的判断与事件记录逻辑清晰、准确且唯一。
    - 采纳性能优化建议，将Pandas列预先转换为Numpy数组。
    """
    ...
    realized_pnl, total_fees = 0.0, 0.0
    ...
    def execute_sell(position, sell_price, timestamp, levels_snapshot, close_price, side="SELL"):
        ...
        trades.append((timestamp, side, round(
            sell_price, 2), position["price"], proceeds, positions_snapshot, levels_snapshot, close_price))
        return True
    ...
    def execute_buy(level_price, buy_price, value_to_invest, timestamp, levels_snapshot, close_price, side="BUY", modify_global_state=True):
        ...
        trades.append((timestamp, side, level_price,
                      f"market@{buy_price:.2f}", cost_before_fee, positions_snapshot, levels_snapshot, close_price))
        return new_position
    ...
    ma_series = df[ma_col_name].to_numpy()
    open_series = df['open'].to_numpy()
    high_series = df['high'].to_numpy()
    low_series = df['low'].to_numpy()
    close_series = df['close'].to_numpy()
    ts_series = df['datetime'].to_numpy()
    ...
    for i in range(1, len(df)):
        ...
        if shift_direction:
            ...
            if shift_direction == "UP":
                ...
                if verbose:
                    print(
                        f"{ts} ▲ 网格上移: {old_levels[0]:.2f}-{old_levels[-1]:.2f} → {lower:.2f}-{upper:.2f}")
                for pos in list(open_positions):
                    execute_sell(pos, c, ts, old_levels,
                                 c, side="SHIFT_UP_SELL")
                total_equity_now = cash
                per_grid_capital_rebuild = total_equity_now / \
                    len(levels) if len(levels) > 0 else 0
                if per_grid_capital_rebuild > 0:
                    for lv in sorted(levels):
                        if lv > c:
                            execute_buy(lv, c, per_grid_capital_rebuild,
                                        ts, levels, c, side="REBUILD_BUY")
            else:  # SHIFT_DOWN
                ...
                surviving_positions = sorted(
                    [p for p in open_positions if p["price"] <= upper], key=lambda p: p['price'], reverse=True)
                positions_to_sell = [
                    p for p in open_positions if p["price"] > upper]
                for pos in positions_to_sell:
                    execute_sell(pos, c, ts, old_levels, c,
                                 side="REDIST_SELL_HIGH")
                surviving_asset_value = sum(
                    p['qty'] * c for p in surviving_positions)
                total_equity_at_shift = cash + surviving_asset_value
                value_per_new_grid = total_equity_at_shift / \
                    len(levels) if len(levels) > 0 else 0
                qty_per_new_grid = value_per_new_grid / c if c > 0 else 0
                new_positions_rebuild = []
                for lv in sorted(levels, reverse=True):
                    qty_needed = qty_per_new_grid
                    if qty_needed <= 1e-8:
                        continue
                    cost_from_survivors, qty_from_survivors = 0.0, 0.0
                    while qty_from_survivors < qty_needed and surviving_positions:
                        pos_to_consume = surviving_positions[0]
                        qty_can_take = min(
                            pos_to_consume['qty'], qty_needed - qty_from_survivors)
                        cost_taken = pos_to_consume['cost'] * (
                            qty_can_take / pos_to_consume['qty']) if pos_to_consume['qty'] > 0 else 0
                        cost_from_survivors += cost_taken
                        qty_from_survivors += qty_can_take
                        pos_to_consume['qty'] -= qty_can_take
                        if pos_to_consume['qty'] < 1e-8:
                            surviving_positions.pop(0)
                    bought_position_part = None
                    qty_to_buy_from_cash = qty_needed - qty_from_survivors
                    if qty_to_buy_from_cash > 1e-8:
                        value_to_invest = qty_to_buy_from_cash * c
                        bought_position_part = execute_buy(
                            lv, c, value_to_invest, ts, levels, c, side="REDIST_BUY_LOW", modify_global_state=False)
                        if bought_position_part is None:
                            continue
                    final_qty = qty_from_survivors + \
                        (bought_position_part['qty']
                         if bought_position_part else 0)
                    final_cost = cost_from_survivors + \
                        (bought_position_part['cost']
                         if bought_position_part else 0)
                    new_positions_rebuild.append(
                        {"price": lv, "qty": final_qty, "cost": final_cost})
                open_positions = new_positions_rebuild
            ...
            if boundary_changed:
                ...
                event_desc = "Grid Shifted & Rebuilt" if shift_direction == "UP" else "Grid Shifted & Redistributed"
                actual_range_str = f"{levels[0]:.2f}-{levels[-1]:.2f}" if levels else "N/A"
                trades.append((ts, f"SHIFT_{shift_direction}", actual_range_str,
                              event_desc, None, positions_snapshot, levels, c))
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "lower_bound": 85000,
        "upper_bound": 120000,
        "step_range": [600, 1000, 1300, 1700, 1800]
    }
            ...
            with pd.ExcelWriter(output_filename, engine='openpyxl') as writer:
                ...
                for i, step_value in enumerate(config["step_range"], 1):
                    ...
                    trade_df = pd.DataFrame(
                        trades, columns=["time", "side", "price", "linked_buy_price", "amount_usdt", "positions", "levels_snapshot", "close_price"])
                    ...
                    buy_trades_count = len(trade_df[trade_df['side'].isin(
                        ['BUY', 'REBUILD_BUY', 'REDIST_BUY_LOW'])])
